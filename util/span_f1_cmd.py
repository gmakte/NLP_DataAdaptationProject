...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Compute span-level F1 metrics for NER tasks.")
    parser.add_argument("gold_path", help="Path to the gold standard annotations file.")
    parser.add_argument("pred_path", help="Path to the predicted annotations file.")
    args = parser.parse_args()

    # Read gold and predicted annotations
    gold_annotations = readNlu(args.gold_path)
    pred_annotations = readNlu(args.pred_path)

    # Compute and display metrics
    metrics = compute_span_f1(gold_annotations, pred_annotations)
    print("\nFinal Metrics:")
    print(metrics)
